...
const stateStorage: Storage = { source: [], destination: [] };

const updateDomProgress = new Map<string, HTMLProgressElement | null>();
const updateDomLabel = new Map<string, HTMLDivElement | null>();

let progressNumberDom = 0;

let downloadUsingLink = localStorage.getItem("EasyBackup-DownloadLink") === "a";
/**
...
export default function CopyFile({ source, destination, options }: Props) {
    ...
    function updateMainProgress() {
        progressNumberDom++;
        if (mainProgress.current) mainProgress.current.value = progressNumberDom;
        if (progressNumberDom === mainProgress.current?.max && operationSpan.current) { // Update also the progress text and the title to completed
    ...
    useEffect((() => {
        ...
        (async () => {
            ...
            async function getDirectory({ handle, sourcePath, isInput }: { handle: FileSystemDirectoryHandle, sourcePath: string, isInput: boolean }) {
                ...
                for await (let entry of handle.values()) entry.kind === "file" ? stateStorage[isInput ? "source" : "destination"].push(`${sourcePath}/${entry.name}`) : await getDirectory({ handle: await handle.getDirectoryHandle(entry.name), sourcePath: `${sourcePath}/${entry.name}`, isInput: isInput })
            }
            await getDirectory({ handle: source, sourcePath: "", isInput: true });
            await getDirectory({ handle: destination, sourcePath: "", isInput: false });
            if (mainProgress.current) mainProgress.current.max = stateStorage.source.length;
    ...
    async function copyItem(progress: number, isForced?: boolean) {
        ...
        let interval;
        try {
            const path = stateStorage.source[progress].substring(1); // Delete the "/" from the string. This is actually unnecessary, since it would be deleted later by the "navigateHandle" function
            if (!path.endsWith(options.fileEnd)) {  // Skip file since it doesn't end with the provided string
                updateMainProgress();
                updateState(prevState => { return { ...prevState, progress: prevState.progress + 1 } })
                return;
            }
            const handle = (await navigateHandle(source, path)).handle;
            const file = await handle.getFile();
            let isDuplicate = stateStorage.destination.indexOf(stateStorage.source[progress]) !== -1;
            ...
            if (isDuplicate) {
                switch (options.duplicates) {
                    ...
                    default: {
                        ...
                        const outputDuplicate = await (await navigateHandle(destination, stateStorage.source[progress].substring(1))).handle.getFile();
            ...
            async function copyCore() {
                interval = setInterval(async () => {
                    const handleObj = await navigateHandle(destination, stateStorage.source[progress].substring(1)); // Get directory handle
                ...
                document.title = `[${progressNumberDom}/${mainProgress.current?.max}] - EasyBackup`;
                if (operationSpan.current) operationSpan.current.textContent = `Copying ${stateStorage.source[progress].substring(1)}`;
                // Obtain the WritableStream and copy the file
                const writableStream = await (await navigateHandle(destination, stateStorage.source[progress].substring(1), true)).handle.createWritable();
                ...
                !isForced && updateState(prevState => { return { ...prevState, copiedFiles: [...prevState.copiedFiles, stateStorage.source[progress].substring(1)] } }); // Update the state, copying the next item
    ...
    useEffect(() => { // Copy the next file, with an effect called only when the "progress" value of the state is updated
        (async () => {
            if (state.progress === -1 || stateStorage.source.length <= state.progress) return;
    ...
    return <>
        <Tip title="Check the files:">
            ...
            <label>Download using:</label><select onChange={(e) => {
                ...
                downloadUsingLink = target.value === "a";
            ...
            <table>
                <tbody id="addFiles">
                    ...
                    {state.showString.map(({ name, edit, size, isDuplicate, entryNumber, duplicateInfo }) => <tr key={`EasyBackup-FileName-${name}-${edit}-${size}-${isDuplicate}-${state.measure}`} style={{ backgroundColor: state.copiedFiles.indexOf(name) !== -1 ? "var(--success)" : isDuplicate && duplicateInfo.hash[0] === duplicateInfo.hash[1] ? "var(--accent)" : isDuplicate ? "var(--attention)" : undefined }}>
                        <td><label className="link" onClick={() => FileRedownload({ path: name, handle: source, useNormalLink: downloadUsingLink })}>{name}</label></td>
                        <td>{isDuplicate ? <>
                            <div role="label" className="topCompare link" onClick={() => FileRedownload({ path: name, handle: source, useNormalLink: downloadUsingLink })}>{new Date(duplicateInfo.lastEdit[0]).toLocaleString()}</div>
                            <div role="label" className="bottomCompare link" onClick={() => FileRedownload({ path: name, handle: destination, useNormalLink: downloadUsingLink })}>{new Date(duplicateInfo.lastEdit[1]).toLocaleString()}</div>
                        ...
                        </div> : <><progress max={100} className="bottomCompare" value={0} ref={el => (updateDomProgress.set(name, el))}></progress>
                            <div role="label" className="bottomCompare" ref={el => (updateDomLabel.set(name, el))}>NA</div>
                        </>}</td>
                    </tr>)}
                </tbody>
            </table>
